...
class HuggingFaceStorage:
    ...
    def create_commit(self, files: list[tuple[Path, str]], message: str) -> None:
        self.ensure_repo()
    ...
    def upload_one(self, local_path: Path, remote_path: str, message: str) -> str:
        self.ensure_repo()
    ...
    def test(self) -> str:
        ...
        return f"Hugging Face bağlantısı başarılı: {who.get('name', 'hesap')}"
...
class AuroraStudio(QMainWindow):
        ...
        def task(progress: Callable[[str, int], None]) -> tuple[dict[str, Any], str, int, int]:
            ...
            try:
                ...
                if request.cover_path:
                    ...
                    remote = f"aurora/artwork/{uuid.uuid4().hex}{ext}"
                ...
                if request.animated_cover_path:
                    ...
                    remote = f"aurora/animated/{uuid.uuid4().hex}{ext}"
                ...
                for index, row in enumerate(request.tracks):
                    ...
                    if settings.upload_master:
                        master_remote = f"aurora/masters/{uuid.uuid4().hex}{row.path.suffix.lower()}"
                        upload_files.append((row.path, master_remote))
                    for variant in variants:
                        ext = variant["path"].suffix.lower()
                        remote = f"aurora/audio/{uuid.uuid4().hex}{ext}"
                    ...
                    if row.atmos_path:
                        remote = f"aurora/atmos/{uuid.uuid4().hex}{row.atmos_path.suffix.lower()}"
                ...
                if upload_files:
                    ...
                    storage.create_commit(upload_files, f"Aurora Music: {request.release_title}")
                else:
    ...
    def upload_asset_to_field(self, field: QLineEdit, category: str) -> None:
        ...
        def task(progress: Callable[[str, int], None]) -> str:
            ...
            url = HuggingFaceStorage(settings).upload_one(path, remote, f"Aurora asset: {category}")
